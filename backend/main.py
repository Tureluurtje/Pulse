...
# When running the file directly (for example from the `backend/` folder in a debugger)
# Python's import machinery won't find the top-level `backend` package because
# sys.path[0] is the `backend/` directory. Add the project root to sys.path so
# absolute imports like `import backend.routes` work regardless of CWD.
